...
import { auth, signInWithEmailAndPassword, signInWithGoogle, signInWithApple, isAuthenticated } from "../../firebase";
...
const LoginPage = memo(() => {
  ...
  const [title, setTitle] = useState("ARY ZAP - Login");
  const Authenticated = isAuthenticated();

  useEffect(() => {
    if (loading) {
      // alert("Loading authentication state...");
      return;
    }
    // alert("Authentication state loaded. User:", user);
    // alert(Authenticated);
    if (Authenticated == true) {
      //   alert("User is authenticated, navigating to home page.");
                ...
                <div className='user-login-card bg-body'>
                  ...
                  <Form action='post'>
                    <Form.Group className='mb-3'>
                      ...
                      <Form.Control type='text' className='rounded-0' value={email} required onChange={(e) => setEmail(e.target.value)} />
                    </Form.Group>
                    <Form.Group className='mb-3'>
                    ...
                    <Form.Group className="text-end mb-3">
                      <Link to="/lost-password" className="text-primary fw-semibold fst-italic">Forgot Password?</Link>
                    </Form.Group>
                    ...
                    <div className="full-button">
                      <div className="iq-button">
                        <Button onClick={() => signInWithEmailAndPassword(email, password)} className="btn text-uppercase position-relative">
                  ...
                  <p className="my-4 text-center fw-500 text-white">New to ZAP?
                    <Link
                      to={{
                        pathname: "/register",
