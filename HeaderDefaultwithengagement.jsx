...
const HeaderDefault = memo(({children}) => {
  ...
  // Track engagement time and session duration
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setEngagementTime((prev) => prev + 1); // increment engagement time every second
  ...
  if (Authenticated) {
    ...
    const sessionDuration = Math.floor((Date.now() - sessionStartTime) / 1000); // Calculate session duration in seconds
...
}, [location, engagementTime, sessionStartTime]); // Add location as a dependency
